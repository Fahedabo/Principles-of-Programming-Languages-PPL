...
import { Result, makeOk, makeFailure, bind, mapResult, mapv, isOk, isFailure } from "../shared/result";
...
const parseGoodBoundExp = (variable: Sexp): Result<BoundExp> =>
    !isIdentifier(variable) ? makeFailure("BoundExp isn't identifier"):
    mapv(parseL4CExp(variable), () => makeBoundExp(makeVarRef(variable)));

const parseGoodTimeExp = (variable: Sexp): Result<TimeExp> =>{
    const r = parseL4CExp(variable)
    if(isFailure(r)){
        return makeFailure("TimeExp isn't ok")
    }
    return makeOk(makeTimeExp(r.value))
}

// HW3: Complete these functions.
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
    (params.length != 1) ? makeFailure("bound? more/less than one argument") :
    parseGoodBoundExp(params[0]);

const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
    (params.length != 1) ? makeFailure("time more/less than one argument") :
...
const unparseBoundExp = (bound: BoundExp):string =>
    `(bound? ${unparse(bound.var)})`;

const unparseTimeExp = (tim: TimeExp):string =>
    `(time ${unparse(tim.exp)})`;
