...
import { map, pair, repeat, zipWith } from "ramda";
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> =>
    isFailure(applyEnv(env, exp.var.var)) ? makeOk(false) :
    makeOk(true)
 
// HW3 complete this function.
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => {
    const start = new Date()
    const res= applicativeEval(exp.exp,env);
    if(isFailure(res)){
    return makeFailure("failure in eval time ")
    }
    const endTime = new Date()
    const timeelapse = endTime.getMilliseconds() - start.getMilliseconds();
    return makeOk(makeCompoundSExp(res.value,timeelapse))
}
