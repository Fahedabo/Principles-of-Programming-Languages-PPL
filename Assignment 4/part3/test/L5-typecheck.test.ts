...
import { isProgram, makeIfExp, makeNumExp, parseL5, parseL5Exp, Program } from '../src/L5/L5-ast';
import { typeofProgram, L5typeof, checkCompatibleType, makeUnion, helper, makegoodarr, typeofIf } from '../src/L5/L5-typecheck';
import { applyTEnv } from '../src/L5/TEnv';
import { isNumTExp, isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeTVar, 
         makeVoidTExp, parseTE, unparseTExp, TExp, isTExp, makeStrTExp, makeUnion2 } from '../src/L5/TExp';
...
describe('L5 Type Checker', () => {
    ...
    describe('L5 Test checkCompatibleType with unions', () => {
        // TODO L51
        it('returns true if both types are compatible', () => {
           // expect().toEqual(makeOk(true));
            expect(checkCompatibleType(makeUnion2(makeNumTExp(),makeBoolTExp()), makeNumTExp(), makeNumExp(6))).toSatisfy(isFailure)
            expect(checkCompatibleType(makeNumTExp(), makeBoolTExp(), makeNumExp(4))).toSatisfy(isFailure);
            expect(checkCompatibleType(makeNumTExp(), makeNumTExp(), makeNumExp(3))).toEqual(makeOk(true));
            expect(checkCompatibleType(makeNumTExp(), makeBoolTExp(), makeNumExp(3))).toSatisfy(isFailure);
            expect(checkCompatibleType(makeUnion2(makeNumTExp(), makeBoolTExp()),makeNumTExp(), makeNumExp(3))).toSatisfy(isFailure);
             expect(checkCompatibleType(makeUnion2(makeBoolTExp(), makeStrTExp()),makeUnion2(makeNumTExp(), makeBoolTExp()), makeNumExp(3))).toSatisfy(isFailure);
             expect(checkCompatibleType(makeUnion2(makeNumTExp(), makeBoolTExp()),makeUnion2(makeBoolTExp(), makeNumTExp()), makeNumExp(3))).toEqual(makeOk(true));
             expect(parseTE("boolean")).toEqual(makeOk(makeBoolTExp()));
        });
    ...
    describe('L5 Test makeUnion', () => {
        ...
        // expect(makeUnionTExp([make]))
        expect(makeUnion(makeNumTExp(), makeBoolTExp())).toEqual(makeUnion( makeBoolTExp(), makeNumTExp()))
        expect(makeUnion(makeNumTExp(), makeBoolTExp())).toEqual(makeUnion(makeBoolTExp(), makeNumTExp()))
        ...
        expect(makeUnion(makeUnion(makeNumTExp(), makeBoolTExp()), makeUnion(makeStrTExp(), makeBoolTExp()))).toEqual(makeUnion( makeStrTExp(), makeUnion(makeNumTExp(), makeBoolTExp())))
        // makeUnion( number, union(number, boolean)) -> union(boolean, number)
        expect(makeUnion(makeNumTExp(), makeUnion(makeNumTExp(), makeBoolTExp()))).toEqual(makeUnion(makeBoolTExp(), makeNumTExp()))
        // TODO L51
        //expect(makeNumTExp()).toEqual(makegoodarr(helper([makeNumTExp(), makeBoolTExp()], [makeStrTExp(), makeBoolTExp()]), []))
        expect(makeUnion(makeNumTExp(), makeNumTExp())).toEqual(makeNumTExp())
        
        

        expect(makeUnion(makeBoolTExp(), makeNumTExp())).toEqual(makeUnion( makeBoolTExp(), makeNumTExp()))
        
        expect(makeUnion(makeNumTExp(), makeUnion2(makeBoolTExp(), makeUnion(makeBoolTExp(), makeNumTExp())))).toEqual(makeUnion(makeBoolTExp(), makeNumTExp()))
    });
    ...
    describe('L5 Test typeOfIf with union in all relevant positions', () => {
        ...
        expect(L5typeof(`(if 1 2 3)`)).toSatisfy(isFailure);
    ...
    describe('L5 Test checkCompatibleType with unions in arg positions of Procedures', () => {
        const exp = makeNumExp(0);
        expect(checkCompatibleType(makeProcTExp([makeUnion2(makeNumTExp(),makeBoolTExp())],makeStrTExp()),makeProcTExp([makeNumTExp()],makeStrTExp()), exp)).toEqual(makeOk(true));
        expect(checkCompatibleType(makeProcTExp([makeNumTExp()],makeStrTExp()),makeProcTExp([makeUnion2(makeNumTExp(),makeBoolTExp())],makeStrTExp()), exp)).toSatisfy(isFailure);
        // // TODO L51
        // // Implement the test for the examples given in the document of the assignment (3.2.4)
        
        expect(isOk(checkCompatibleType(makeProcTExp([makeUnion2(makeNumTExp(),makeBoolTExp())],makeStrTExp()),makeProcTExp([makeNumTExp()],makeStrTExp()), makeNumExp(0)))).toEqual(true);
        // expect(isOk(checkCompatibleType(makeProcTExp([makeNumTExp()],makeStrTExp()),makeProcTExp([makeNumTExp()],makeStrTExp())))).toEqual(true);
        // expect(isOk(checkCompatibleType(makeProcTExp([makeUnionTExp([makeNumTExp(),makeBoolTExp()])],makeStrTExp()),makeProcTExp([makeUnionTExp([makeNumTExp(),makeBoolTExp()])],makeStrTExp())))).toEqual(true);
         expect(isFailure(checkCompatibleType(makeProcTExp([makeNumTExp()],makeStrTExp()),makeProcTExp([makeUnion2(makeNumTExp(),makeBoolTExp())],makeStrTExp()), makeNumExp(0)))).toEqual(true);

        //expect(checkCompatibleType(makeUnion2(makeProcTExp([makeNumTExp(), makeNumTExp()], makeNumTExp()), makeProcTExp([makeNumTExp(), makeNumTExp()], makeNumTExp())), makeBoolTExp(), exp)).toEqual(makeOk(true));
