...
export const make2 = (arr: TExp[]): UnionTExp =>
({tag: "UnionTExp", components : arr})
export const makeUnion2 = (te1: TExp, te2: TExp): TExp =>
    // Replace return type and body with appropriate code.
    !isUnionTExp(te1) ? (!isUnionTExp(te2) ? (equals(te1, te2) ? isAtomicTExp(te1) ? te1 : make2([te1, te2]): make2(makegoodarr2([te1, te2], []))) :
                        make2(makegoodarr2(cons(te1, te2.components), []))) :
    !isUnionTExp(te2) ? make2(makegoodarr2(cons(te2, te1.components), [])) :
    make2(makegoodarr2(helper2(te1.components, te2.components),[]))
    

export const helper2 = (te1: TExp[], te2: TExp[]): TExp[] => 
te1.length === 1 ? cons(te1[0], te2) : cons(te1[0], helper2(te1.slice(1), te2))

export const makegoodarr2 = (te: TExp[], te2: TExp[]): TExp[] =>
te.length === 1 ? makegood2(te[0], te2) : makegoodarr2(te.slice(1), makegood2(te[0], te2));
const makegood2 = (te: TExp, te2: TExp[]): TExp[] => ///taking an element and checkes if it is in te2 it will not add else will add it
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    isArray(texp) ? checkIfUnion(texp) :
    makeFailure(`Unexpected TExp - ${format(texp)}`);

export const parseTExpUion = (texp: Sexp): Result<TExp> =>
    (texp === "number") ? makeOk(makeNumTExp()) :
    (texp === "boolean") ? makeOk(makeBoolTExp()) :
    (texp === "void") ? makeOk(makeVoidTExp()) :
    (texp === "string") ? makeOk(makeStrTExp()) :
...
const checkIfUnion = (texps: Sexp[]): Result<TExp> =>{
    const pos = texps.indexOf('->');
    return (pos === -1) ? parsUnionTExp(texps) :
           parseCompoundTExp(texps, pos);
}

const parsUnionTExp = (texps: Sexp[]): Result<TExp> =>{
    if(isNonEmptyList<Sexp>(texps)){
    const First = first(texps);
    const Rest : Result<TExp[]> = mapResult((x) => parseTExpUion(x), rest(texps)) 
    
    return  (First !== "union") ? makeFailure("The first arg is not the word union") :
            (Rest.tag === "Failure") ? makeFailure("Rest.tag is failure") : 
            Rest.value.length != 2 ? makeFailure("the args of union are not two") :
            makeOk(makeUnion2(Rest.value[0], Rest.value[1])) // the sort part was in makeUnion2
    }
    return makeFailure("Empty texps");
    

}

const parseCompoundTExp = (texps: Sexp[], pos: number): Result<ProcTExp> => {
    
    return (pos === -1)  ? makeFailure("jnjlk") :
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseUnion = (paramTes: TExp[]): Result<string[]> => 
        isNonEmptyList<TExp>(paramTes) ? bind(unparseTExp(first(paramTes)), (paramTE: string) =>
            mapv(mapResult(unparseTExp, rest(paramTes)), (paramTEs: string[]) =>
                cons(paramTE,  paramTEs))) :
        makeOk(["Empty"]);

    const unparseunion = (array: TExp[]): Result<string | string[]> =>{
        if(array.length<2){
            return makeFailure("the length is not good")
        }
        const arg0 = unparseTExp(array[0])
        const arg1 = unparseTExp(array[1])
        const arg2 = unparseunion(array.slice(1))
        if(array.length > 2){
            if(isOk(arg0)){
                if(isOk(arg2))
                return makeOk(`(union ${arg0.value} ${arg2.value})`)
            }
        }
        else{
            if(isOk(arg0)){
                if(isOk(arg1))
                return makeOk(`(union ${arg0.value} ${arg1.value})`)
            }
        }
        
        return makeFailure("bhjn")
    }
